...
int16_t SYS_GetCustomTemperatureLevel(void)
{
  /* USER CODE BEGIN SYS_GetTemperatureLevel_1 */
  ...
  APP_LOG(TS_OFF, VLEVEL_M, "ADC rawValue: %d\r\n", measuredLevel);

  RxCmd.tempread= ReadTemperatureSensor();

  // return (int16_t) RxCmd.tempread;
...
static uint32_t ADC_ReadChannels(uint32_t channel)
{
  ...
  {
	  APP_LOG(TS_OFF, VLEVEL_M, "In If ADC channel: %d\tADC rawValue: %d\r\n", channel, ADCxConvertedValues);
  }
  else
  {
	  APP_LOG(TS_OFF, VLEVEL_M, "In Else part ADC channel: %d\r\n", channel);
  }
